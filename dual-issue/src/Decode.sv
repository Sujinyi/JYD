...
module Decode(
    ...
    logic discard_fetch1_w;
    logic discard_fetch1_uncond_w;
    ...
    // Keep the upper instruction after conditional branches. If the branch is
    // eventually taken, the execute-stage flush will clear it. Only
    // unconditional control transfers must discard the sibling instruction
    // immediately.
    assign discard_fetch1_uncond_w =
        (fetch_out0_inst[6:0] == 7'h6f) || // JAL
        (fetch_out0_inst[6:0] == 7'h67);   // JALR
    assign discard_fetch1_w =
        fetch_out0_accept && fetch_out0_valid && discard_fetch1_uncond_w;
